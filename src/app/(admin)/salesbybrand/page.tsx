...
export default async function SalesByBrandPage() {
    ...
    const { SalesByBrand: playCovers } = salesQueries({ dateInit: today, dateEnd: today, emp: "3", brands: ['PEINING', 'KIMASTER', 'B-MAX', 'INOVA', 'DEVIA', 'HREBOS',] })
    ...
    const [respSalesPlayCell, respSalesPlayCustom, respSalesPlayUp, respPlayCovers, respStock, respDebt] = await Promise.all([
    ...
    const salesData = [...respSalesPlayCell.data.returnObject.body, ...respSalesPlayCustom.data.returnObject.body, ...respSalesPlayUp.data.returnObject.body, ...respPlayCovers.data.returnObject.body]
