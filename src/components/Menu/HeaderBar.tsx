...
const HeaderBar = ({ isOpen }: menuProp) => {
    const { setThemeColor, location: url } = ColorSchemeController();
    const firstPart = url.pathname.split("/")[1];
    const secondPart = url.pathname.split("/")[2];

    return (
        <div className="p-4 xl:ml-60 2xl:ml-80 bg-[#edf3fb]">
            <nav
                className={`block w-full max-w-full ${setThemeColor()} text-white shadow-none rounded-xl transition-all px-3 py-2 overflow-auto`}
            >
