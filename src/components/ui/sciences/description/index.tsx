        ...
        <>
            {data.map((item, index) => (
                <div key={index} className={`w-full ${handleSelection && 'cursor-pointer'}`}>
                    <div onClick={() => handleSelection && handleSelection(item[dataKey] as unknown as string, index, vibrantPalette[index % vibrantPalette.length])} className="flex w-36 px-2 items-center">
