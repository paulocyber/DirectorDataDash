...
export const goalsQueries = ({ id, surname, dateInit, month, year }: QueryProps) => {
  ...
  v_funcionarios fnc ON fnc.id_pss = mti.ID_FNC WHERE fnc.ID_EMP IN (1, 2, 3, 4) ${surname ? `AND fnc.APELIDO_PSS like '%${surname}%'` : `mti.ID_FNC = ${id} OR mti.ID_FNC IS NULL)`} AND mtv.DATA_INICIO_MTA = 
  '${dateInit}'`;
