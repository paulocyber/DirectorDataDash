...
export const billsToPayQueries = ({
  ...
  let billetPaidAndOpen = `select pgm.id_pgm AS ID_SDS, trim(CASE WHEN pgm.status_pgm = 1 THEN 'Em Aberto' WHEN pgm.status_pgm = 2 THEN 'Pago' WHEN pgm.status_pgm = 4 THEN 'Parcial' ELSE 'Desconhecido' END) AS status_pgm, pgm.data_vencimento_pgm, pgm.valor_pgm, pgm.VALOR_PAGO_PGM, pgm.numero_documento_pgm, pgm.id_grc || ' - ' || pgm.descricao_grc AS grupo_centro, pgm.nome_pss,
