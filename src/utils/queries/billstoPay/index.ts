...
export const billsToPayQueries = ({
  ...
  '${dateEnd}' AND CAST(pgm.datahora_lancamento_pgm AS DATE) BETWEEN '2022-12-01' AND CURRENT_DATE) AND (pgm.status_pgm = 1 OR pgm.status_pgm = 4) AND pgm.id_emp in(1, 2, 3, 5, 100 ) order by pgm.data_vencimento_pgm, pgm.id_pss`;
  ...
  '${year}/${month}/${day}' AND CAST(pgm.datahora_lancamento_pgm AS DATE) BETWEEN '2022-12-01' AND CURRENT_DATE) AND (pgm.status_pgm = 1 OR pgm.status_pgm = 4) AND pgm.id_emp in(1, 2, 3, 5, 100 ) order by pgm.data_vencimento_pgm, pgm.id_pss`;
  ...
  let billetPaidAndOpen = `select pgm.id_pgm AS ID_SDS, pgm.status_pgm, pgm.data_vencimento_pgm, pgm.valor_pgm, pgm.VALOR_PAGO_PGM, pgm.numero_documento_pgm, pgm.id_grc || ' - ' || pgm.descricao_grc AS grupo_centro, pgm.nome_pss,
   ...
   AS DATE) BETWEEN '2022-12-01' AND CURRENT_DATE AND pgm.status_pgm IN (1, 4)) OR (pgm.datahora_pagamento_pgm BETWEEN DATE '${dateInit}' AND DATE '${dateEnd}' AND pgm.status_pgm = 2)) AND pgm.id_emp in(1, 2, 3, 5, 100 ) ORDER BY pgm.data_vencimento_pgm,
    pgm.id_pss`;
