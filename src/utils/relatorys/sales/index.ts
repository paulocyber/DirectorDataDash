...
// Tipagem
import { ItemsGoalProgress, ItemsProfitsFromSale } from "@/types/sales";
type CreatePdf = {
  pageSize: PageSize;
...
interface SalesPdfProps {
  ...
  profitSales: ItemsProfitsFromSale[];
}
...
export default function SalesPdf({
  ...
  const formattedSales = profitSales.map((sale) => ({
    ...sale,
    ...
    content: [
      {
        columns: [
          {
            text: `Meta ${company}: ${formatCurrency(goalProgress[2].value)}`,
            fontSize: 10,
            bold: true,
            margin: [0, 10, 0, 5],
            alignment: "center",
          ...
          {
            text: `Total lucro: ${formatCurrency(goalProgress[1].value)}`,
            fontSize: 10,
            bold: true,
            margin: [0, 10, 0, 5],
            alignment: "center",
          ...
          {
            text: `Vendas: ${formatCurrency(goalProgress[0].value)}`,
            fontSize: 10,
            bold: true,
            margin: [0, 10, 0, 5],
            alignment: "center",
