...
export const billsToPayQueries = ({
  ...
  const formattedCostsCenters = Array.isArray(costsCenters)
    ? costsCenters.map((costCenter) => `'${costCenter}'`).join(", ")
    : "";
  ...
  pgm.nome_pss, pgm.id_cnt || ' - ' || pgm.descricao_cnt AS centro_custo, LIST(DISTINCT COALESCE(frm.descricao_frm, pgm.descricao_frm)) AS descricao_frm FROM v_pagamentos pgm LEFT JOIN pagamentos_itens pgi ON 
  pgi.id_pgm = pgm.id_pgm LEFT JOIN formas_pagamentos frm ON frm.id_frm = pgi.ID_FRM WHERE ( (CAST(pgm.data_vencimento_pgm AS DATE) BETWEEN '${dateInit}' AND '${dateEnd}'  AND CAST(pgm.datahora_lancamento_pgm AS DATE) 
  BETWEEN '2022-12-01' AND CURRENT_DATE  AND pgm.status_pgm IN (1, 4)) OR  (pgm.datahora_pagamento_pgm BETWEEN DATE '${dateInit}' AND DATE '${dateEnd}'  AND pgm.status_pgm IN (2, 4))) AND pgm.id_emp IN (1, 2, 3, 4, 5, 
    100) ${
      costsCenters && costsCenters.length > 0
        ? `and pgm.ID_CNT in (${formattedCostsCenters})`
        : ""
    } GROUP BY  pgm.id_pgm, pgm.status_pgm, pgm.data_vencimento_pgm, pgm.valor_pgm, pgm.VALOR_PAGO_PGM, pgm.numero_documento_pgm, pgm.id_grc, pgm.descricao_grc, pgm.nome_pss, pgm.id_cnt, 
    pgm.descricao_cnt ORDER BY pgm.data_vencimento_pgm`;

  let expiredBillet = `select pgm.restante_pgm, pgm.id_cnt||' - '||pgm.descricao_cnt as centro_custo from v_pagamentos pgm where (CAST(pgm.data_vencimento_pgm AS DATE) BETWEEN '${dateInit}' AND '${dateEnd}' AND 
    CAST(pgm.datahora_lancamento_pgm AS DATE) BETWEEN '2022-12-01' AND CURRENT_DATE) AND (pgm.status_pgm = 1 OR pgm.status_pgm = 4) AND pgm.id_emp in(1, 2, 3, 4, 5, 100 ) order by pgm.data_vencimento_pgm, pgm.id_pss`;
