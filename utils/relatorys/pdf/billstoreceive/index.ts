...
export default async function BillsToReceivePdf({
  ...
  const getMostRecentPaidBills = (bills: ItemsBillsToReceive[]) => {
  ...
  const totalReceipt = sumValuesByKey(openBills, (bill) => bill.RESTANTE_RCB);

  const sortedOpenBills = [...billetsOpenData].sort((a, b) => {
